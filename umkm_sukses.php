...
// Ambil data level UMKM 2 (Sukses)
$sql = "SELECT * FROM umkm WHERE level_umkm = 2 ORDER BY created_at DESC LIMIT $start, $limit";
$result = mysqli_query($conn, $sql);

// Hitung total data untuk pagination
$countSql = "SELECT COUNT(*) AS total FROM umkm WHERE level_umkm = 2";
$countResult = mysqli_query($conn, $countSql);
$total = mysqli_fetch_assoc($countResult)['total'];
$total_pages = ceil($total / $limit);

$kategori = "UMKM Sukses";
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<div class="card">
    <a href="export_umkm_sukses.php" class="export-btn">Export ke Excel</a>
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            while($row = mysqli_fetch_assoc($result)){
                echo "<tr>
                    ...
                    <td>
                        <a href='editdata.php?id={$row['id']}&redirect=umkm_sukses.php' class='btn btn-edit'>Edit</a>
...
function confirmDelete(id){
    Swal.fire({
        ...
        if(result.isConfirmed){
            // tambahkan level=2
            window.location.href = "delete.php?id=" + id + "&level=2";
        }
    });
}
